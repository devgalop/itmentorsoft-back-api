...
@pytest.mark.asyncio
async def test_get_contents_by_category_topic_when_repository_returns_items_then_should_return_success():
    ...
    content_repository.get_resource_contents_by_category_and_related_topic = AsyncMock(
        return_value=PaginatedResourceContentResult(items=items, total=1)
    )

    handler = GetContentsByCategoryTopicHandler(content_repository)
    request = GetContentsByCategoryTopicPaginationRequest(
        category="novice", topic="Python", page=0, page_size=10
    )
...
@pytest.mark.asyncio
async def test_get_contents_by_category_topic_when_repository_returns_empty_then_should_return_empty_list():
    content_repository = AsyncMock()
    content_repository.get_resource_contents_by_category_and_related_topic = AsyncMock(
    ...
    handler = GetContentsByCategoryTopicHandler(content_repository)
    request = GetContentsByCategoryTopicPaginationRequest(
        category="unknown", topic="unknown", page=0, page_size=10
    )
    response = await handler.handle(request)
...
@pytest.mark.asyncio
async def test_get_contents_by_category_topic_when_custom_pagination_then_should_forward_params():
    content_repository = AsyncMock()
    content_repository.get_resource_contents_by_category_and_related_topic = AsyncMock(
        return_value=PaginatedResourceContentResult(items=[], total=0)
    )
    ...
    handler = GetContentsByCategoryTopicHandler(content_repository)
    request = GetContentsByCategoryTopicPaginationRequest(
        category="emerging", topic="SQL", page=3, page_size=25
    )
...
@pytest.mark.asyncio
async def test_get_contents_by_category_topic_when_multiple_items_then_should_return_correct_total():
    content_repository = AsyncMock()
    items = [
        ResourceContentResponse(
            content_id=f"id{i}",
    ...
    handler = GetContentsByCategoryTopicHandler(content_repository)
    request = GetContentsByCategoryTopicPaginationRequest(
        category="proficient", topic="Golang", page=0, page_size=5
    )
    response = await handler.handle(request)
